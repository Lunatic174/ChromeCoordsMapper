function FindCoords(selector) {
    ...
    var degMinRegex = /(\s|\>|^)((?:(?:([-+])|([NS])\s?)?([1-8]?\d)\°\s?(?:([0-5]?\d(?:\.\d{1,4})?)[\'\′])?|(90)\°\s?(?:(0{1,2}(?:\.0{1,4})?)[\'\′])?)\s?([NS]?))(\D+?.*?)((?:([-+])|([EW])\s?)?(?:(180)\°\s?(?:(0{1,2}(?:\.0{1,4})?)[\'\′])?|(?:(1[0-7]\d)|(0?\d?\d))\°\s?(?:([0-5]?\d(?:\.\d{1,4})?)[\'\′])?)\s?([EW]?))/gmi;
    var degMinSecRegex = /(\s|\>|^)((?:([-+])|([NS])\s?)?(?:([1-8]?\d)\°\s?(?:([0-5]?\d)[\'\′]\s?(?:([0-5]?\d(?:\.\d{1,4})?)[\"\″])?)?|(90)\°\s?(?:(0{1,2})[\'\′]\s?(?:(0{1,2}(?:\.0{1,4})?)[\"\″])?)?)\s?([NS]?))(\D+?.*?)((?:([-+])|([EW])\s?)?(?:(180)\°\s?(?:(0{1,2})[\'\′]\s?(?:(0{1,2}(?:\.0{1,4})?)[\"\″])?)?|(?:(1[0-7]\d)|(0?\d?\d))\°\s?(?:([0-5]?\d)[\'\′]\s?(?:([0-5]?\d(?:\.\d{1,4})?)[\"\″])?)?)\s?([EW]?))/gmi;
    var degMinSecDotRegex = /(\s|\>|^)((?:([-+])|([NS])\s?)?(?:([1-8]?\d)\.\s?(?:([0-5]?\d)\.\s?([0-5]?\d(?:\.\d{1,4})?)?)?|(90)\.\s?(?:(0{1,2})\.\s?(0{1,2}(?:\.0{1,4})?)?)?)\s?([NS]?))(\D+?.*?)((?:([-+])|([EW])\s?)?(?:(180)\.\s?(?:(0{1,2})\.\s?(0{1,2}(?:\.0{1,4})?)?)?|(?:(1[0-7]\d)|(0?\d?\d))\.\s?(?:([0-5]?\d)\.\s?([0-5]?\d(?:\.\d{1,4})?)?)?)\s?([EW]?))/gmi;

    $(selector).each(function() {
        var $content = $(this);
        $content.html($content.html().replace(degMinSecDotRegex, "$1<span class='coords' data-coords='deg-min-sec' data-lat-hem='$3$4$11' data-lat-deg='$5$8' data-lat-min='$6$9' data-lat-sec='$7$10' data-lng-hem='$14$15$23' data-lng-deg='$16$19$20' data-lng-min='$17$21' data-lng-sec='$18$22'><span class='latitude'>$2</span>$12<span class='longitude'>$13</span></span>"));
